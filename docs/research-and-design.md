...
从产品使用路径看，所有工程能力被收拢为三个模块：评测集共创 Agent 负责把散乱 question/answer 变成经过确认的评测集；模型自动验收 Runner 负责调用算法团队提供的 Endpoint 并完成批量评分；产品洞察 Agent 负责把逐题结果变成发布决策、失败簇、责任层和回归清单。PM 的核心动作只有“确认标准”和“消费洞察”，不需要介入批量执行过程。
...
产品经理不需要逐题写复杂打分提示，而要负责四件事：确认场景、写清期望行为、标红不可上线行为、确定阈值。Agent 负责把散乱线上案例变成上述结构，并标出需要事实专家介入的缺口。算法团队收到的不是一个模糊总分，而是问题码、证据、失败簇、建议责任层和可直接回归的案例 ID。
...
当前离线 deterministic Judge 用于演示端到端自动化，不替代语义 Judge。OpenAI adapter 和通用 command adapter 已提供，但需要调用方配置密钥或内部命令。真实工具类案例若没有 trace/最终状态，只能进入 unknown 或被红线规则保守处理。HTML 报告是单文件，可归档和发送；Web 工作台展示同一份生成结果。
